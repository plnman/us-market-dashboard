...
class EnhancedSmartMoneyScreener:
    ...
    def get_analyst_ratings(self, ticker: str) -> Dict:
        ...
        try:
            ...
            # Upside potential - Handle zeroes and None
            if current_price and current_price > 0 and target_price and target_price > 0:
                upside = ((target_price / current_price) - 1) * 100
            else:
                upside = 0
            
            # Use 'N/A' for display if invalid
            target_display = round(target_price, 2) if target_price and target_price > 0 else 0
    ...
    def calculate_composite_score(self, row: pd.Series, tech: Dict, fund: Dict, analyst: Dict, rs: Dict) -> Tuple[float, str]:
        ...
        # Determine grade
        if composite >= 80: grade = "S (Strong Buy)"
        elif composite >= 70: grade = "A (Buy)"
        elif composite >= 60: grade = "B (Accumulate)"
...
def main():
    ...
    if not results.empty:
        print(f"\\n[RESULTS] TOP {args.top} ENHANCED SMART MONEY PICKS")
